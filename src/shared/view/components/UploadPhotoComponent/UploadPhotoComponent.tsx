...
class UploadPhotoComponent extends React.PureComponent<TProps> {
			...
			<AvatarUploadDropzone onAvatarImageDrop={onUpload}>
				{children ? (
					...
					<div className={b({ [size]: true })}>
						<div className={b('upload-zone', { error: !!hasError })}>{this.renderContent()}</div>
					</div>
				)}
	...
	private renderContent() {
		...
		if (uploadProgress && uploadProgress > 0) {
			return <div className={b('upload-progress')}>{uploadProgress} %</div>;
		}

		if (uploadedImageUrl) {
			return <Image className={b('uploaded-image')} src={uploadedImageUrl} />;
		}
