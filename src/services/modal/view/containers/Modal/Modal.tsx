...
class Modal extends React.Component<TProps, IState> {
		...
		return (
			<Draggable handle={`.${b('header').toString()}`} bounds={`.${b()}`}>
				<div className={b('content', { closing, 'as-card': !disableCard }).toString()}>
					...
					>
						{icon && <div className={b('content-top-left')}>{this.renderIcon()}</div>}
					...
					{actions && <div className={b('content-actions')}>{actions}</div>}
				</div>
			</Draggable>
		);
	}
	...
	private handleClose() {
		...
		this.setState({ closing: true }, () => setTimeout(() => this.props.onClose(), closeTimeout));
	}
...
const withRedux = connect<IStateProps, {}, IOwnProps>(Modal.mapStateToProps)(Modal);
