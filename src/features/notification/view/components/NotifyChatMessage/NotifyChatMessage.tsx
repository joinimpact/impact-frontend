...
import {
  IConversationMessageResponseItem, IRequestHoursMessage,
  IStandardMessage,
  IVolunteerRequestAcceptance, IVolunteerRequestProfileMessage,
...
class NotifyChatMessage extends React.PureComponent<TProps> {
      ...
      <div className={b()}>
        {conversation && (
          <div className={b('left')}>
            {this.renderConversationAvatar(conversation)}
          </div>
        )}
        <div className={b('content')}>
          {conversation && (
            <div className={b('content-head')}>
              {conversation.name}
            </div>
          )}
  ...
  private renderHoursAccepted(body: IRequestHoursMessage) {
    ...
    return this.wrappedText(t('NOTIFY-SERVICE:EVENT:MESSAGE_HOURS_ACCEPTED', {
  ...
  private renderHoursDeclined(body: IRequestHoursMessage) {
    const { translate: t, conversation } = this.props;
    return this.wrappedText(t('NOTIFY-SERVICE:EVENT:MESSAGE_HOURS_DECLINED', {
      userName: conversation ? conversation.name : '',
    }));
  }
  ...
  private renderHoursRequested(body: IRequestHoursMessage) {
    const { translate: t, conversation } = this.props;
    return this.wrappedText(t('NOTIFY-SERVICE:EVENT:MESSAGE_HOURS_REQUESTED', {
      userName: conversation ? conversation.name : '',
    }));
  }
  ...
  private renderVolunteerRequestProfile(body: IVolunteerRequestProfileMessage) {
    ...
    return this.wrappedText(t('NOTIFY-SERVICE:EVENT:MESSAGE_VOLUNTEER_REQUEST_PROFILE', {
  ...
  private renderRequestAcceptance(body: IVolunteerRequestAcceptance) {
    const { translate: t } = this.props;
    return this.wrappedText(t('NOTIFY-SERVICE:EVENT:VOLUNTEER_REQUEST_ACCEPTANCE', {
