...
class InviteTeamMembersForm extends React.PureComponent<TProps, IState> {
	...
	public render() {
		...
		return (
			<div className={b()}>
				<div className={b('caption')}>{t('INVITE-TEAM-MEMBERS-FORM:STATIC:CAPTION')}</div>
				<div className={b('subtitle')}>{t('INVITE-TEAM-MEMBERS-FORM:STATIC:TITLE')}</div>

				<div className={b('team-label')}>{t('INVITE-TEAM-MEMBERS-FORM:STATIC:TEAM')}</div>
				{this.state.members.length === 0 && (
					<div className={b('team-sublabel')}>{t('INVITE-TEAM-MEMBERS-FORM:STATIC:TEAM-PLACEHOLDER')}</div>
				)}
