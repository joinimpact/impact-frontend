...
class EventResponsesPortalComponent extends React.PureComponent<TProps> {
	public render() {
		...
		return <div className={b()}>{eventResponses.map(this.renderResponseItem)}</div>;
	}
			...
			<div className={b('item')} key={`item-${index}`} onClick={this.handleClick.bind(this, response)}>
				<div className={b('item-image')}>
					{response.profilePicture ? (
						...
						<UserAvatar className={b('item-user-avatar')} firstName={response.firstName} lastName={response.lastName} />
					)}
				</div>
