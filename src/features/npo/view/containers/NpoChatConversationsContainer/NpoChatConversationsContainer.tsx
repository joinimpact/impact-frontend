...
class NpoChatConversationsContainer extends React.PureComponent<TProps> {
	...
	private renderContent() {
		...
		if (loadConversationsCommunication.error) {
			return (
				<ErrorScreen
					title={t('NPO-CHAT-CONVERSATIONS-CONTAINER:ERROR:LOAD-CONVERSATIONS-FAILED-TITLE')}
					message={loadConversationsCommunication.error}
				/>
			);
		}
				...
				<div className={b('search-bar')}>
					<div className={b('search-bar-content')}>
						<SearchInput
							...
							size={20}
						/>
						<Button color="grey">{t('NPO-CHAT-CONVERSATIONS-CONTAINER:ACTION:NEW')}</Button>
					</div>
				</div>
				<div className={b('conversations')}>
					<CustomScrollbar>
						<div className={b('conversations-content')}>{this.props.conversations.map(this.renderConversationRow)}</div>
					</CustomScrollbar>
				</div>
			</>
		);
		...
		return (
			...
			>
				...
				<div className={b('conversation-content')}>
					<div className={b('conversation-content-name')}>{conversation.name}</div>
