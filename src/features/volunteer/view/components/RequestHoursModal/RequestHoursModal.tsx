...
class RequestHoursModal extends React.PureComponent<TProps> {
	public render() {
		...
		return (
			...
			>
				<div className={b()}>
					<div className={b('hint')}>{t('REQUEST-HOURS-MODAL:STATIC:TEXT')}</div>
					<div className={b('row')}>
						<Label htmlFor={fieldNames.hours}>{t('REQUEST-HOURS-MODAL:LABEL:HOURS')}</Label>
					...
					<div className={b('row')}>
						<Label htmlFor={fieldNames.description}>{t('REQUEST-HOURS-MODAL:LABEL:DESCRIPTION')}</Label>
						<div className={b('field')}>
							<InputBaseFieldWrapper
								component={InputBaseField}
	...
	private wrapModalContent(modalContent: React.ReactNode) {
		return <form onSubmit={this.handleRequestHoursSubmit}>{modalContent}</form>;
	}
	...
	private renderActions() {
		...
		return (
			<div className={b('actions')}>
				...
				<Button type="submit" color="blue" isShowPreloader={this.props.communication.isRequesting}>
