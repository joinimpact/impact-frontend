...
class TodayTasksComponent extends React.PureComponent<TProps> {
	public render() {
		...
		return (
			<div className={b()}>
				<div className={b('title')}>{t('TODAY-TASKS-COMPONENT:STATIC:TODAY')}</div>
				<div className={b('content')}>{this.renderTodayTasks()}</div>
			</div>
		);
	}
				...
				<div key={`task-${index}`} className={b('task')}>
					<div className={b('task-left-part')}>
						...
						<div className={b('task-left-part-title')}>
							<div className={b('task-left-part-title-content')}>{task.title}</div>
						</div>
					</div>
