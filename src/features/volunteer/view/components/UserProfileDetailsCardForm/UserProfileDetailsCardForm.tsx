...
class UserProfileDetailsCardForm extends React.PureComponent<TProps> {
	public render() {
		...
		return <div className={b()}>{editMode ? this.renderEditableCard() : this.renderReadonlyCard()}</div>;
	}
		...
		return (
			<div className={b('readonly-card')}>
				<div className={b('readonly-card-row')}>
					<div className={b('readonly-card-row-label')}>{t('USER-PROFILE-DETAILS-CARD-FORM:LABEL:NAME')}</div>
					<div className={b('readonly-card-row-value')}>{`${values.firstName} ${values.lastName}`}</div>
				</div>

				<div className={b('readonly-card-row')}>
					<div className={b('readonly-card-row-label')}>{t('USER-PROFILE-DETAILS-CARD-FORM:LABEL:EMAIL')}</div>
					<div className={b('readonly-card-row-value')}>{values.email}</div>
				</div>

				<div className={b('readonly-card-row')}>
					<div className={b('readonly-card-row-label')}>{t('USER-PROFILE-DETAILS-CARD-FORM:LABEL:LOCATION')}</div>
					<div className={b('readonly-card-row-value')}>{values.address.description}</div>
				</div>

				<div className={b('readonly-card-row')}>
					<div className={b('readonly-card-row-label')}>{t('USER-PROFILE-DETAILS-CARD-FORM:LABEL:BIRTHDAY')}</div>
					<div className={b('readonly-card-row-value')}>{moment(values.birthday).format(defaultDateFormat)}</div>
				</div>
			</div>
		);
	}
