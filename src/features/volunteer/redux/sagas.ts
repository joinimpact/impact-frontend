...
function* executeSaveVolunteerPersonalInfo(
	{ api, notify, translate: t }: IDependencies,
	{ payload }: NS.ISaveVolunteerPersonalInfo,
) {
	try {
		...
		notify.notifyError(
			t('USER-EDIT-PROFILE-CONTAINER:NOTIFY:SAVED-ERROR', {
				error: message,
			}),
		);
		...
		while (true) {
			...
			if (task) {
				const currentConversation: IConversationResponseItem | null = yield select(
					volunteerChatSelectors.selectCurrentConversation,
				);
